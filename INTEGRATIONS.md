...
**otaverify** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `otaverify` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
otaverify ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
otaverify ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
otaverify ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `otaverify` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="otaverify") for rec in records]   # records = your JSON output
