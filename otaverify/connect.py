"""Native cognis-connect emit for otaverify — forward findings to any platform.

Maps otaverify's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    otaverify ... --format json | otaverify-emit --to stix
    otaverify-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    """Tailor a otaverify record toward the Finding contract before normalize().
